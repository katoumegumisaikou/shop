...
import { RegisterByPhone, SendSmsCode } from '@/services/auth'
...
export default function RegisterPage() {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      setCode(res.code) // 开发期自动填入验证码，上线后移除此行
      Taro.showToast({ title: '验证码已发送', icon: 'success' })
    } catch (err: any) {
      Taro.showToast({ title: err.message || '发送失败，请重试', icon: 'none' })
    }
  }
  ...
  const handleRegister = async () => {
    ...
    setLoading(true)
    try {
      await RegisterByPhone(phone, password, code)
    ...
    <View className='login-page'>
      <View className='login-card'>
        <View className='login-card__header'>
          <Text className='login-card__title'>账号注册</Text>
          <Text className='login-card__description'>注册后即可畅享购物</Text>
        </View>
